const { groupdb , userdb,cmd, jsonformat,send, botpic, TelegraPh, RandomXP, Config, tlang, warndb, sleep,getAdmin,getBuffer, prefix } = require('../lib')
const { Sticker, createSticker, StickerTypes } = require("wa-sticker-formatter");
const moment = require("moment-timezone");
const Levels = require("discord-xp");
const fs = require('fs-extra')
const Jimp = require("jimp");
const Nithya = require('../lib/commands')
...
async(Nithya, msg, text,{isCreator}) => {
...
cmd({
 ...
 async(Nithya, msg, text,{ isCreator }) => {
      ...
      let metadata = await Nithya.bot.groupMetadata(msg.chat);
      var ppuser;
      let num = msg.sender;
      ...
      try {  ppuser = await Nithya.bot.profilePictureUrl(num, 'image') }catch { ppuser = 'https://telegra.ph/file/93f1e7e8a1d7c4486df9e.jpg' ; }
      return await Nithya.bot.sendMessage(msg.chat, { image: { url: ppuser }, caption: welcome_messages,mentions: [num] } )
 ...
 async(Nithya, msg, text,{ isCreator }) => {
    ...
    try {  ppuser = await Nithya.bot.profilePictureUrl(num, 'image') }catch { ppuser = 'https://telegra.ph/file/93f1e7e8a1d7c4486df9e.jpg' ; }
        ...
        return await Nithya.bot.sendMessage(msg.chat, { image: { url: ppuser }, caption: goodbye_messages, mentions: [num] })
...
async(Nithya, msg, text,{ isCreator }) => {
    if (!msg.isGroup) return msg.reply(tlang().group);
    const groupMetadata = msg.isGroup ? await Nithya.bot.groupMetadata(msg.chat).catch((e) => {}) : "";
